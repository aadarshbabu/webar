...
AFRAME.registerComponent("model-controller", {
            ...
            try {
              ...
              if (!isNaN(max) && max !== Infinity && max > 0.1) {
                ...
                this.el.setAttribute(
                  "scale",
                  `${this.currScale} ${this.currScale} ${this.currScale}`
                );
                this.enableAction = true;
              }
            } catch (error) {
  ...
  wheel: function (evt) {
    if (this.enableAction) {
      ...
      if (evt.deltaY < 0) {
        // bigger
        if (this.currScale + this.scaleStep < this.maxScale)
          this.currScale += this.scaleStep;
      } else {
        if (this.currScale - this.scaleStep > this.minScale)
          this.currScale -= this.scaleStep;
      }
      window.assetParam.scale = this.currScale;
